...
<section class="container mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-10">
  <div class="relative rounded-xl bg-[var(--color-surface,transparent)] p-6 ring-1 ring-black/5 dark:ring-white/10">
    ...
    {#if items.length > 1}
      <button class="absolute left-2 top-1/2 -translate-y-1/2 rounded bg-black/40 p-2 text-white hover:bg-black/60" onclick={prev} aria-label="Previous">‹</button>
      <button class="absolute right-2 top-1/2 -translate-y-1/2 rounded bg-black/40 p-2 text-white hover:bg-black/60" onclick={next} aria-label="Next">›</button>
    {/if}
  </div>
</section>
{/if}